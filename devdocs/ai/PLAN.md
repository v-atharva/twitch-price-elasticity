...
- **M1** synthetic panel with known elasticity + estimator validation: CS recovers truth, TWFE biased, placebos null (`make synth`, `make test`)
- **M2** cohort/price reference table with provenance + anchor tests
- **M3** scraper infrastructure (Playwright driver, cache, Wayback module, roster builder)
- **M4** pilot pull ~300 channels (TR+MX, BR, US controls) → coverage report → **decision gate: scale to 1–5k channels vs descope to 2-cohort study**
- **M5** real panel through the same code path as synthetic; R `did` cross-check in CI
- **M6** estimation: event study (−12..+12, uniform bands), cohort/overall ATTs, elasticity ± CI, TWFE + Goodman-Bacon contrast
- **M7** diagnostics: pre-trends + Rambachan–Roth honest-DiD, placebos (fake dates, followers outcome, permutation), robustness (not-yet-treated controls, drop-one-cohort, alternative audience assignment, winsorization, size subsamples)
- **M8** README for a hiring manager + RESULTS.md for a non-technical stakeholder
